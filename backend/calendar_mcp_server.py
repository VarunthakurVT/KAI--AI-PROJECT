"""
KAI – Calendar MCP Server

A simple HTTP server that provides calendar functionality for the main app.
Exposes endpoints for checking availability and booking events.

Run with: python calendar_mcp_server.py
Then set CALENDAR_MCP_URL=http://localhost:3333 in your backend .env
"""
...
class CheckCalendarRequest(BaseModel):
    ...
    duration_minutes: int = Field(120, description="Min continuous free time in minutes")
...
class BookEventRequest(BaseModel):
    """Request to book a calendar event."""
...
class BookEventResponse(BaseModel):
    """Response after booking an event."""
    success: bool
...
def get_booked_slots(date_obj: datetime, tz_name: str) -> List[tuple]:
    ...
    booked = []
    calendar_id = "primary"  # Demo uses single calendar
...
async def book_event(request: BookEventRequest) -> BookEventResponse:
    ...
    try:
        ...
        event = {
            ...
            "htmlLink": f"http://localhost:3333/event/{event_id}",  # Mock link
        }
...
if __name__ == "__main__":
    ...
    print("Starting KAI Calendar MCP Server on http://localhost:3333")
